#!/usr/bin/env python3
"""
Convert PNG moon phase icons to EPD47 4-bit grayscale bitmap format at 100x100.
"""
...
def png_to_epd47_bitmap(png_path, output_size=100):
    """
    ...
    lines.append(f"const uint8_t moon_{icon_name_c}_{output_size}[] = {{")
...
def main():
    moon_dir = "/Users/leighherbert/Desktop/weather_icons/moon"
    output_file = "src/moon_phase_bitmaps.h"
    ...
    for icon in icons:
        png_path = os.path.join(moon_dir, icon)
        if os.path.exists(png_path):
            print(f"Converting {icon}...")
            bitmap_code = png_to_epd47_bitmap(png_path, output_size=100)
